...
//  printf("TERMINAL FILE INDEX:%d\n",OFTable[FDs].fileIndex);
...
int isValidDirEntry(DirEntry* dirEntry)
{
  ...
  if (!error)
  {
   ...
   time(&a);
   b = localtime(&a);
   if (dirEntry->date.year > b->tm_year + 1900 - 1980) error = 1;
	...
	switch (test)
	{
		...
		case 3:
		{
			...
			for (i=0; i<numWords; i++)
			{
				...
				try(fmsReadFile(tFID[i], buf, strlen(text[i])));
        if (debug) printf("\n READ %s from %d",buf, tFID[i]);
				// write to buffer
				try(fmsWriteFile(tFID[test2], buf, strlen(text[i])));
        if (debug) printf("\n write %d of %s to %s",strlen(text[i]), buf, buf2);
				// close word file
				if (debug) printf("\n  fmsCloseFile(%d)", tFID[i]);
				try(fmsCloseFile(tFID[i]));
				// close test2 file
...
//    ERR67 = End of Directory
{
	...
	int dirCluster = dir;
//  printf("Made it to getdent name = %.8s\n",mask);
...
//My ugly hack of the previous function :)
int fmsSetDirEntry(char* mask, DirEntry* dirEntry)
{
  char buffer[BYTES_PER_SECTOR];
  int dirIndex, dirSector, error;
  int dirNum = 0;  
  ...
  { // load directory sector
    ...
    { // read directory entry
      ...
      if (curEntry.name[0] == 0) return ERR67; // EOD
      dirNum++;                            // prepare for next read
      if (curEntry.name[0] == 0xe5);         // Deleted entry, go on...
      else if (curEntry.attributes == LONGNAME);
      else if (fmsMask(mask, curEntry.name, curEntry.extension))
      {
